...
// Split into smaller, targeted queries to avoid timeouts
const queries = [
  {
    name: 'US National Historic Landmarks (with images)',
  ...
  {
    name: 'US National Historic Landmarks (without images)',
    country: 'United States',
    sparql: `SELECT ?item ?itemLabel ?coord ?image ?article ?desc WHERE {
      ?item wdt:P1435 wd:Q15243209 .
      ?item wdt:P625 ?coord .
  ...
  {
    name: 'Canadian National Historic Sites (no images)',
    country: 'Canada',
    sparql: `SELECT ?item ?itemLabel ?coord ?image ?article ?desc WHERE {
      ?item wdt:P1435 wd:Q1568856 .
      ?item wdt:P625 ?coord .
  ...
  {
    name: 'US Historic Districts',
    country: 'United States',
    sparql: `SELECT ?item ?itemLabel ?coord ?image ?article ?desc WHERE {
      ?item wdt:P31 wd:Q5765735 .
      ?item wdt:P625 ?coord .
      ?item wdt:P18 ?image .
      OPTIONAL { ?article schema:about ?item ; schema:isPartOf <https://en.wikipedia.org/> }
      OPTIONAL { ?item schema:description ?desc . FILTER(LANG(?desc) = "en") }
...
function guessCategory(name, desc) {
  ...
  if (/church|cathedral|chapel|abbey|priory|monastery|mission|basilica|synagogue|mosque/i.test(text)) return 'religious';
  if (/battle|military|war|army|navy|memorial|cemetery/i.test(text)) return 'cultural';
  if (/archaeological|pueblo|mound|petroglyph|cliff dwelling|native|indigenous|prehistoric|ancient/i.test(text)) return 'ancient';
  if (/mill|mine|bridge|canal|railway|lighthouse|harbour|dam|factory|industrial/i.test(text)) return 'industrial';
  if (/park|monument|canyon|volcano|cave|reef|forest|wilderness|desert|glacier|geyser|lake|mountain/i.test(text)) return 'natural';
  return 'cultural';
}

async function run() {
  console.log('=== North America Import v2 ===\n');
  ...
  for (let i = 0; i < allResults.length; i++) {
    ...
    if ((i + 1) % 200 === 0) console.log(`${i + 1}/${allResults.length} (${inserted} inserted, ${skipped} skipped)`);
  }
  
  console.log(`\n✅ Inserted ${inserted} sites (${skipped} skipped)`);
